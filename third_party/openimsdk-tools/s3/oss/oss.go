...
func (o *OSS) AccessURL(ctx context.Context, name string, expire time.Duration, opt *s3.AccessURLOption) (string, error) {
	...
	// 桃花岛补丁:阿里云 OSS 出于安全,签名 GET URL 禁止覆盖 response-content-type
	// (错误 0017-00000902 "Can not override response header on content-type",无 Bucket 开关)。
	// 强制清空 ContentType,只保留 response-content-disposition(文件名,OSS 允许),
	// 否则私有桶(publicRead:false)IM 图片/文件拉取一律 400。
	if opt != nil {
		opt.ContentType = ""
	}
	var opts []oss.Option
